  ...
  >
    ...
    <ol-control-bar>
      <ol-control-toggle
        ...
        className="edit"
        title="Point"
        :onToggle="(active: any) => changeDrawType(active, 'Point')"
      />
      <ol-control-toggle
        :order="1"
        ...
        className="edit"
        title="Polygon"
        :onToggle="(active: any) => changeDrawType(active, 'Polygon')"
      />
      <ol-control-toggle
        :order="2"
...
export default {
  setup() {
    ...
    const selectConditions = inject('ol-selectconditions') as any
    ...
    const extent = inject('ol-extent') as any

    const Feature = inject('ol-feature') as any
    const Geom = inject('ol-geom') as any

    const contextMenuItems = ref([]) as any
    const vectorsource = ref(null) as any
    const view = ref(null) as any

    const drawEnable = ref(false)
    const drawType = ref('Point')

    const changeDrawType = (active: any, draw: any) => {
    ...
    const swipeControl = ref(null) as any
    const jawgLayer = ref(null) as any
    const osmLayer = ref(null) as any
